...
def solve_images(driver):
    WebDriverWait(driver, 10).until(ExpectedConditions.presence_of_element_located((By.ID, "rc-imageselect-target")))
    ...
    tile1 = WebDriverWait(driver, 10).until(
        ExpectedConditions.element_to_be_clickable((By.XPATH, '//div[@id="rc-imageselect-target"]/table/tbody/tr[{0}]/td[{1}]'.format(
...
def main_program_loop():
    ...
    while end_num >= start_num:
        ...
        jsonFile = file_path + "/json/"+ str(start_numformat) + ".json2"
        ...
        if check_exists_by_xpath(driver, '//h4[text()="Almost done"]'):
            ...
            driver.execute_script("alert('Please manual solve Captcha within 25 second');")
            time.sleep(25)
